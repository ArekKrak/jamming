...
export default function SearchBar({ onSearch }) {
    ...
    const [isSearching, setIsSearching] = useState(false); // NEW: disable during pending request
    ...
    return (
        ...
        <form onSubmit={handleSubmit}>
            <input 
                ...
                disabled={isSearching}                  // lock input while searching
            />
            {/* Avoid incorrectly formatted requests (common errors to avoid) */}
            <button type="submit" disabled={isSearching || String(query).trim() === ""}>{isSearching ? "Searching..." : "Search"}</button> {/* Disable the button when the input is empty or only 
